...
class VannaBase(ABC):
    ...
    def get_sql_prompt(
        ...
        **kwargs,
        ...
        import os
        initial_prompt_md_path = os.path.join(os.getcwd(), r"doc\initial_prompt.md")
        with open(initial_prompt_md_path, "r", encoding="utf-8") as f:
            initial_prompt_md = f.read()
        initial_prompt += (
            initial_prompt_md.format(dialect=self.dialect)
        )
        ...
        for database in df[database_column].unique().tolist():
            for schema in (
                ...
                for table in (
                    ...
                    doc += df_columns_filtered_to_table[columns].to_markdown(index=False)
